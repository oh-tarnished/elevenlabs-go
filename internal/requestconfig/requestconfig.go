...
import (
	...
	"github.com/oh-tarnished/elevenlabs-go/internal"
	"github.com/oh-tarnished/elevenlabs-go/internal/apierror"
...
func getDefaultHeaders() map[string]string {
	return map[string]string{
		"User-Agent": fmt.Sprintf("elevenlabs/Go %s", internal.PackageVersion),
	}
}
...
func getPlatformProperties() map[string]string {
	return map[string]string{
		"X-Request-Lang":            "go",
		"X-Request-Package-Version": internal.PackageVersion,
		"X-Request-OS":              getNormalizedOS(),
		"X-Request-Arch":            getNormalizedArchitecture(),
		"X-Request-Runtime":         "go",
		"X-Request-Runtime-Version": runtime.Version(),
	}
}
...
func NewRequestConfig(ctx context.Context, method string, u string, body any, dst any, opts ...RequestOption) (*RequestConfig, error) {
	...
	req.Header.Set("X-Request-Retry-Count", "0")
	req.Header.Set("X-Request-Timeout", "0")
	...
	if req.Header.Get("X-Request-Timeout") == "0" {
		if cfg.RequestTimeout == time.Duration(0) {
			req.Header.Del("X-Request-Timeout")
		} else {
			req.Header.Set("X-Request-Timeout", strconv.Itoa(int(cfg.RequestTimeout.Seconds())))
		}
	}
...
func (cfg *RequestConfig) Execute() (err error) {
	...
	shouldSendRetryCount := cfg.Request.Header.Get("X-Request-Retry-Count") == "0"
	...
	for retryCount := 0; retryCount <= cfg.MaxRetries; retryCount += 1 {
		...
		if shouldSendRetryCount {
			req.Header.Set("X-Request-Retry-Count", strconv.Itoa(retryCount))
		}
