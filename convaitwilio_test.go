...
func TestConvaiTwilioOutboundCallWithOptionalParams(t *testing.T) {
	...
	_, err := client.Convai.Twilio.OutboundCall(context.TODO(), elevenlabs.ConvaiTwilioOutboundCallParams{
		AgentID:            "agent_id",
...
func TestConvaiTwilioRegisterCallWithOptionalParams(t *testing.T) {
	t.Skip("Prism tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Convai.Twilio.RegisterCall(context.TODO(), elevenlabs.ConvaiTwilioRegisterCallParams{
		AgentID:    "agent_id",
		...
		Direction: elevenlabs.ConvaiTwilioRegisterCallParamsDirectionInbound,
		XiAPIKey:  elevenlabs.String("xi-api-key"),
	})
	if err != nil {
		var apierr *elevenlabs.Error
